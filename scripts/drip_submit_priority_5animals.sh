#!/usr/bin/env bash
# Drip-submit configs/priority_lora_qwen_default_5animals.yaml under the
# per-user QOSMaxSubmitJobPerUserLimit (~250 pending+running jobs).
#
# Submits the priority-tier 5-animal LoRA sweep
#   5 animals x 10 ranks x 6 gen_seeds x 1 train_seed = 300 experiments
# (a substantial fraction is already cached from lora_canonical_default_fill.yaml
#  at gen_seeds in {1, 42, 123} for cat/eagle/owl/wolf — submit.sh skips those)
# as a sequence of cache-aware array jobs:
#
...
#
# This handles preemption/failed tasks transparently because submit.sh
# only schedules array indices that still have uncached experiments.
#
# Run on fe02 in the background:
#   nohup bash scripts/drip_submit_priority_5animals.sh \
#     > logs/drip_submit_priority_5animals.log 2>&1 &
#
# To tail progress:
#   tail -f logs/drip_submit_priority_5animals.log
#
# Prerequisites (run once before launching):
#   ./submit.sh generate-datasets  --config configs/priority_lora_qwen_default_5animals.yaml
...
CONFIG="configs/priority_lora_qwen_default_5animals.yaml"
ARRAY_SIZE=60           # ~150 fresh experiments / 60 tasks ~= 2-3 experiments/task
