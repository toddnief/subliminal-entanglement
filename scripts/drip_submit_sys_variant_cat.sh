#!/usr/bin/env bash
# Drip-submit configs/sys_variant_cat.yaml under the per-user
# QOSMaxSubmitJobPerUserLimit (~250 pending+running jobs).
#
# Submits the cat sys_variant sweep (819 experiments = 13 variants × 7 ranks
# × 3 gen_seeds × 3 train_seeds; ~567 still uncached at submission time) as a
# sequence of cache-aware array jobs:
...
#   nohup bash scripts/drip_submit_sys_variant_cat.sh \
#     > logs/drip_submit_sys_variant_cat.log 2>&1 &
#
# To tail progress:
#   tail -f logs/drip_submit_sys_variant_cat.log
...
CONFIG="configs/sys_variant_cat.yaml"
ARRAY_SIZE=150          # 819 experiments / 150 tasks ≈ 5-6 experiments/task
MAX_GPUS=6
...
submit_round() {
    # Returns 0 with $LAST_SUBMIT_OUT populated, or non-zero on failure.
    local attempt=0
    while (( attempt < 5 )); do
