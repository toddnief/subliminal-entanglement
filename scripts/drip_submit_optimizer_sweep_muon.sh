#!/usr/bin/env bash
# Drip-submit configs/optimizer_sweep_muon_4animals.yaml under the
# per-user QOSMaxSubmitJobPerUserLimit (~250 pending+running jobs).
#
# Submits the Muon rank sweep
#   4 animals x 10 ranks x 3 gen_seeds x 1 train_seed = 120 experiments
# (all fresh — no muon runs in the canonical adamw cache) as a sequence
# of cache-aware array jobs:
#
...
#   nohup bash scripts/drip_submit_optimizer_sweep_muon.sh \
#     > logs/drip_submit_optimizer_sweep_muon.log 2>&1 &
#
# To tail progress:
#   tail -f logs/drip_submit_optimizer_sweep_muon.log
#
# Prerequisites (run once before launching):
#   ./submit.sh generate-datasets  --config configs/optimizer_sweep_muon_4animals.yaml
#   ./submit.sh generate-baselines --config configs/optimizer_sweep_muon_4animals.yaml
#
...
# are shared byte-for-byte with priority_lora_qwen_default_5animals.yaml, so
# generate-datasets should be a no-op if the priority sweep has already run.
# Baselines hash on student_model + eval_prompts only — they're already cached
# from any prior Qwen2.5-7B run on the same eval prompt list.
...
CONFIG="configs/optimizer_sweep_muon_4animals.yaml"
